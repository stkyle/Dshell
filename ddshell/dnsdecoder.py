...
class DNSDecoder(dshell.TCPDecoder):
    """extend DNSDecoder to handle DNS request/responses
            ...
            to allow for detection of DNS spoofing, etc.. (multiple responses 
            to request with same ID)
    ...
    def packetHandler(self, udp, data):
        ...
        if addr[0][1] < addr[1][1]:
            # swap ports if source port is lower, to keep tuple (client,server)
            addr = addr[1], addr[0]
            ...
            for requesttime, request, responsecount in self.requests[conn].values():
                if not responsecount:
                    if type(conn) is tuple:
                        conn = dshell.Packet(self, conn)  # wrap UDP addresses
